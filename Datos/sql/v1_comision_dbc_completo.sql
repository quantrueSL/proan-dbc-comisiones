-- =============================================================================
-- Comisiones DBC — base: consulta del senior. Cambios: filtro de cancelaciones
-- real (cancel_lg no sirve, siempre NULL), fix de 'PISO' -> 'VTA EN PISO',
-- y los 4 escalones de CEDIS (el senior solo tenía el primero).
-- =============================================================================
CREATE OR REPLACE TABLE `proan-quantrue.ZZ_PRUEBAS.dbc_comisiones_calculadas` AS

WITH

-- Material -> SET. 2 materiales (18345, 19012) traen 2 SETs; ANY_VALUE evita duplicar línea.
sets AS (
  SELECT
...
-- CEDIS, 3 escalones (del más específico al más flojo). Sin el escalón de
-- mapeo manual por nombre -- ese es hardcodeado, se excluye a propósito.
cedis AS (
  SELECT
...
-- Tarifas oficiales (fuente confirmada: proan_ZTSD_OV_COM_*_20260829). Sin cambios vs. el senior.
tarifas_h AS (
  SELECT
    WERKS, LGORT, VKBUR,
...
-- Universo de facturas. Cambios vs. el senior:
--   - document_category = 'M' en vez de cancel_lg (ese campo siempre es NULL, no filtraba nada)
--   - billing_date con techo (excluye años inválidos tipo 2201/2202)
facturas AS (
  SELECT
    f.billing_document,
    f.item_number,
...
-- Tarifa por SET+canal/tipo_venta. Único cambio vs. el senior: 'PISO' -> 'VTA EN PISO'
-- (dm_cedis nunca trae 'PISO' a secas, así que esas 4 líneas nunca cruzaban).
con_tarifa AS (
  SELECT
    *,
    CASE
...
SELECT
  ...
  bukrs,
  gsber           AS division,
  vkbur           AS oficina_ventas,
  lgort           AS almacen,
  werks           AS planta,
  matnr_clean     AS matnr,
  SETNAME         AS set_material,
  tipo_venta,
  canal,
  cantidad,
  importe_mxn,
  tarifa,
  ROUND(cantidad * tarifa, 2) AS comision_mxn,
  -- tarifa primero: en L (y A) la tarifa no depende de tipo_venta, así que
  -- puede haber comisión calculada aunque tipo_venta sea NULL -- eso es OK,
  -- no SIN_CEDIS.
  CASE
    WHEN tarifa     IS NOT NULL THEN 'OK'
    WHEN SETNAME    IS NULL     THEN 'SIN_SET'
    WHEN tipo_venta IS NULL     THEN 'SIN_CEDIS'
    ELSE 'SIN_TARIFA'
  END AS status
FROM con_tarifa
ORDER BY billing_date DESC, billing_document, item_number;
